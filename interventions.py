...
def get_quar_inds(quar_policy, sim):
    ...
    if   quar_policy is None:    quar_test_inds = np.array([])
    elif quar_policy == 'start': quar_test_inds = znu.true(sim.agents.human.date_quarantined==t-1) # Actually do the day after since testing usually happens before contact tracing
    elif quar_policy == 'end':   quar_test_inds = znu.true(sim.agents.human.date_end_quarantine==t+1) # +1 since they are released on date_end_quarantine, so do the day before
    elif quar_policy == 'both':  quar_test_inds = np.concatenate([znu.true(sim.agents.human.date_quarantined==t-1), znu.true(sim.agents.human.date_end_quarantine==t+1)])
    elif quar_policy == 'daily': quar_test_inds = znu.true(sim.agents.human.quarantined)
        ...
        quar_test_inds = np.unique(np.concatenate([znu.true(sim.agents.human.date_quarantined==t-1-q) for q in quar_policy]))
...
class test_num(Intervention):
    ...
    **Examples**::
    ...
    def __init__(self, daily_tests, symp_test=100.0, quar_test=1.0, quar_policy=None, subtarget=None,
        ...
        self.start_day   = start_day
        self.end_day     = end_day
        self.pdf         = znu.get_pdf(**sc.mergedicts(swab_delay)) # If provided, get the distribution's pdf -- this returns an empty dict if None is supplied
        return
    ...
    def apply(self, sim):
        ...
        symp_inds = znu.true(sim.agents.human.symptomatic)
        symp_test = self.symp_test
        if self.pdf: # Handle the onset to swab delay
            symp_time = znd.default_int(t - sim.agents.human.date_symptomatic[symp_inds]) # Find time since symptom onset
        ...
        if self.ili_prev is not None:
            if rel_t < len(self.ili_prev):
                n_ili = int(self.ili_prev[rel_t] * sim['pop_size_by_type']['human'])  # Number with ILI symptoms on this day
                ili_inds = znu.choose(sim['pop_size_by_type']['human'], n_ili) # Give some people some symptoms. Assuming that this is independent of COVID symptomaticity...
        ...
        diag_inds  = znu.true(sim.people.diagnosed)
        ...
        if sim.rescale_vec[t]/sim['pop_scale'] < 1: # We still have rescaling to do
            ...
            out_pop_tot_prob = sim.scaled_pop_size - sim.rescale_vec[t]*sim['pop_size_by_type']['human'] # Find out how many people are missing and assign them each weight 1
        ...
        test_inds = znu.choose_w(probs=test_probs, n=n_tests, unique=True) # Choose who actually tests
        sim.agents.human.test(test_inds, test_sensitivity=self.sensitivity, loss_prob=self.loss_prob, test_delay=self.test_delay)
...
class test_prob(Intervention):
    ...
    **Examples**::
    ...
    def apply(self, sim):
        ...
        pop_size = sim['pop_size_by_type']['human'] # Total population size
        ili_inds = []
        if self.ili_prev is not None:
            rel_t = t - start_day
            if rel_t < len(self.ili_prev):
                n_ili = int(self.ili_prev[rel_t] * pop_size)  # Number with ILI symptoms on this day
                ili_inds = znu.choose(pop_size, n_ili) # Give some people some symptoms, assuming that this is independent of COVID symptomaticity...
                ili_inds = np.setdiff1d(ili_inds, symp_inds)
        ...
        diag_inds       = znu.true(sim.agents.human.diagnosed)

        # Construct the testing probabilities piece by piece -- complicated, since need to do it in the right order
        test_probs = np.zeros(sim['pop_size_by_type']['human']) # Begin by assigning equal testing probability to everyone
        ...
        test_inds = znu.true(znu.binomial_arr(test_probs)) # Finally, calculate who actually tests

        # Actually test people
        sim.agents.human.test(test_inds, test_sensitivity=self.sensitivity, loss_prob=self.loss_prob, test_delay=self.test_delay) # Actually test people
    ...
    **Example**::
    ...
    def initialize(self, sim):
        ...
        if sc.isnumber(self.trace_probs):
            ...
            self.trace_probs = {k:val for k in sim.agents.layer_keys()} 
        if sc.isnumber(self.trace_time):
            val = self.trace_time
            self.trace_time = {k:val for k in sim.agents.layer_keys()}
        return
    ...
    def select_cases(self, sim):
        ...
        if not self.presumptive:
            inds = znu.true(sim.agents.human.date_diagnosed == sim.t) # Diagnosed this time step, time to trace
        else:
            just_tested = znu.true(sim.agents.human.date_tested == sim.t) # Tested this time step, time to trace
            inds = znu.itruei(sim.agents.human.exposed, just_tested) # This is necessary to avoid infinite chains of asymptomatic testing
    ...
    def identify_contacts(self, sim, trace_inds):
        ...
        for lkey, this_trace_prob in self.trace_probs.items():
            ...
            traceable_inds = sim.agents.contacts[lkey].find_contacts(trace_inds)
            if len(traceable_inds):
                contacts[self.trace_time[lkey]].extend(znu.binomial_filter(this_trace_prob, traceable_inds)) # Filter the indices according to the probability of being able to trace this layer

        array_contacts = {}
        for trace_time, inds in contacts.items():
            array_contacts[trace_time] = np.fromiter(inds, dtype=znd.default_int)
    ...
    def schedule_notification_and_quarantine(self, sim, contacts):
        ...
        '''
        is_dead = znu.true(sim.agents.human.dead) # Find people who are not alive
        for trace_time, contact_inds in contacts.items():
            ...
            sim.agents.human.known_contact[contact_inds] = True
            sim.agents.human.date_known_contact[contact_inds] = np.fmax(sim.agents.human.date_known_contact[contact_inds], sim.t + trace_time)
            sim.agents.human.schedule_quarantine(contact_inds, start_date=sim.t + trace_time, period=self.quar_period - trace_time)  # Schedule quarantine for the notified people to start on the date they will be notified
        return
...
class notify_contacts(Intervention):
    ...
    **Example**::
    ...
    def select_cases(self, sim):
        ...
        inds = znu.true(sim.agents.human.date_diagnosed == sim.t)
    ...
    def schedule_notification(self, sim, contacts):
        ...
        for trace_time, contact_inds in contacts.items():
            contact_inds = np.setdiff1d(contact_inds, is_dead) # Do not notify contacts who are dead
            sim.agents.human.known_contact[contact_inds] = True
            sim.agents.human.date_known_contact[contact_inds] = np.fmax(sim.agents.date_known_contact[contact_inds], sim.t + trace_time)
...
class simple_vaccine(Intervention):
    ...
    **Examples**::
    ...
    def __init__(self, days, prob=1.0, rel_sus=0.0, rel_symp=0.0, subtarget=None, cumulative=False, **kwargs):
        ...
        self.cumulative = np.array(cumulative, dtype=znd.default_float) # Ensure it's an array
        return
    ...
    def initialize(self, sim):
        ...
        super().initialize()
        self.days = process_days(sim, self.days)
        self.doses             = np.zeros(sim.n, dtype=znd.default_int) # Number of doses given per person of this vaccine
        self.vaccination_dates = [[]] * sim.n # Store the dates when people are vaccinated
        self.orig_rel_sus      = sc.dcp(sim.agents.human.rel_sus) # Keep a copy of pre-vaccination susceptibility
        self.orig_symp_prob    = sc.dcp(sim.agents.human.symp_prob) # ...and symptom probability
        self.mod_rel_sus       = np.ones(sim.n, dtype=znd.default_float) # Store the final modifiers
        self.mod_symp_prob     = np.ones(sim.n, dtype=znd.default_float) # Store the final modifiers
        self.vacc_inds         = None
        return
    ...
    def apply(self, sim):
        ...
        for ind in find_day(self.days, sim.t, interv=self, sim=sim):
            ...
            vacc_inds = znu.true(znu.binomial_arr(vacc_probs)) # Calculate who actually gets vaccinated
            ...
            sim.agents.human.rel_sus[vacc_inds]   *= rel_sus_eff
            sim.agents.human.symp_prob[vacc_inds] *= rel_symp_eff
            ...
            sim.agents.human.vaccinated[vacc_inds] = True
            sim.agents.human.doses[vacc_inds] += 1
...
class BaseVaccination(Intervention):
    ...
    def _parse_vaccine_pars(self, vaccine=None):
        ...
        if isinstance(vaccine, str):
            ...
            choices, mapping = znpar.get_vaccine_choices()
            variant_pars = znpar.get_vaccine_variant_pars()
            dose_pars = znpar.get_vaccine_dose_pars()
    ...
    def initialize(self, sim):
        ...
        default_variant_pars = znpar.get_vaccine_variant_pars(default=True)
        default_dose_pars    = znpar.get_vaccine_dose_pars(default=True)
        ...
        if 'target_eff' in self.p.keys():
            ...
            if self.p['doses'] == len(self.p['target_eff']):
                ...
                VE_symp = znimm.calc_VE_symp(2**nabs, sim.pars['immunity_pars']['human']['nab_eff'])
        ...
        self.doses = np.zeros(sim['pop_size'], dtype=znd.default_int) # Number of doses given per person
    ...
    def vaccinate(self, sim, vacc_inds, t=None):
        ...
        # Perform checks
        vacc_inds = vacc_inds[~sim.agents.human.dead[vacc_inds]] # Skip anyone that is dead
        ...
        prior_vacc = znu.true(sim.agents.human.vaccinated)
        ...
        if len(vacc_inds):
            ...
            sim.agents.human.vaccinated[vacc_inds] = True
            sim.agents.human.vaccine_source[vacc_inds] = self.index
            sim.agents.human.doses[vacc_inds] += 1
            sim.agents.human.date_vaccinated[vacc_inds] = t
            znimm.update_peak_nab(sim.agents.human, vacc_inds, self.p)
            ...
            if t >= 0: # Only record these quantities by default if it's not a historical dose
                ...
                sim.agents.human.flows['new_doses']      += len(vacc_inds)*factor # Count number of doses given
                sim.agents.human.flows['new_vaccinated'] += len(new_vacc)*factor # Count number of people not already vaccinated given doses
...
def process_sequence(sequence, sim):
    ...
    if callable(sequence):
        sequence = sequence(sim.agents.human)
    elif sequence == 'age':
        sequence = np.argsort(-sim.agents.human.age)
    ...
    def select_people(self, sim):
        ...
        if sim.t >= np.min(self.days):
            ...
            for _ in find_day(self.days, sim.t, interv=self, sim=sim):
                ...
                vacc_probs = np.zeros(sim['pop_size_by_type'])  # Initialize vaccination probabilities

                # Find eligible people
                vacc_probs[znu.true(sim.agents.human.dead)] *= 0.0  # Do not vaccinate dead people
                ...
                if self.booster:    eligible_inds = sc.findinds(sim.agents.human.vaccinated)
                else:               eligible_inds = sc.findinds(~sim.agents.human.vaccinated)
    ...
    **Example**::
    ...
    def select_people(self, sim):
        ...
        if self._scheduled_doses[sim.t]:
            scheduled = np.fromiter(self._scheduled_doses[sim.t], dtype=znd.default_int) # Everyone scheduled today
            scheduled = scheduled[(self.doses[scheduled] < self.p['doses']) & ~sim.agents.human.dead[scheduled]] # Remove anyone who's already had all doses of this vaccine, also dead people
            ...
            scheduled = np.array([], dtype=znd.default_int)
        ...
        vacc_probs[znu.true(sim.agents.human.dead)] = 0.0  # Dead people are not eligible
        ...
        # If this is a booster, exclude unvaccinated people; otherwise, exclude vaccinated people
        if self.booster: vacc_probs[znu.false(sim.agents.human.vaccinated)] = 0.0
        else:            vacc_probs[znu.true(sim.agents.human.vaccinated)]  = 0.0 # Anyone who's received at least one dose is counted as vaccinated
        ...
        first_dose_eligible = self.sequence[znu.binomial_arr(vacc_probs[self.sequence])]
...
class historical_vaccinate_prob(BaseVaccination):
    ...
    **Example**::
    ...
    def initialize(self, sim):
        ...
        # extend nab profiles
        self.extra_days = np.abs(np.min(self.days).astype(znd.default_int))
        new_nab_length = sim.npts + self.extra_days
        if new_nab_length > len(sim.pars['immunity_pars']['human']['nab_kin']):
            sim.pars['immunity_pars']['human']['nab_kin'] = znimm.precompute_waning(length=new_nab_length, pars=sim.pars['immunity_pars']['human']['nab_decay'])
            if sim.agents.human:
                sim.people.pars['immunity_pars']['human']['nab_kin'] = sim.pars['immunity_pars']['human']['nab_kin']
        ...
        seed_inds = znu.true(sim.agents.human.date_exposed == 0)
        sim.agents.human.make_naive(seed_inds)
        ...
        for t in times: # step through time, init flows, including zeroing out the seed infections.
            ...
            sim.agents.human.t = t
            to_update = znu.true(self.doses > 0)  # Update nabs for anyone vaccinated using this intervention
            if len(to_update):
                znimm.update_nab(sim.agents.human, inds=to_update)

        # reset the seed infections
        sim.agents.human.infect(seed_inds, layer='seed_infection')
        return
    ...
    def select_people(self, sim, t=None):
        ...
        if t >= np.min(self.days):
            ...
            for _ in find_day(self.days, t, interv=self, sim=sim):
                vacc_probs = np.zeros(sim['pop_size_by_type']['human'])
                unvacc_inds = sc.findinds(~sim.agents.human.vaccinated)
                if self.subtarget is not None:
                    subtarget_inds, subtarget_vals = get_subtargets(self.subtarget, sim)
                    vacc_probs[subtarget_inds] = subtarget_vals  # People being explicitly subtargeted
                else:
                ...
                vacc_inds = znu.binomial_filter(self.compliance[0], vacc_inds)
                ...
                if len(vacc_inds):
                    if self.p.interval is not None:
                        ...
                        if next_dose_day < (sim['n_days'] + self.extra_days):
                            ...
                            second_dose_vacc_inds = znu.binomial_filter(self.compliance[1], vacc_inds)
...
class historical_wave(Intervention):
    ...
    **Example**::
    ...
    def apply(self, sim):
        ...
        # Check that the simulation parameters are correct
        if not sim['immunity_pars']['human']['use_waning']: # pragma: no cover
            errormsg = 'zn.historical_wave() requires use_waning=True for human agents. Please enable waning.'
            raise RuntimeError(errormsg)
        if sim['rescale'] and sim['pop_scale'] > 1:
            errormsg = 'zn.historical_wave() requires rescale=False, since rescaling assumes non-included agents are naive. Please disable dynamic rescaling.'
            raise RuntimeError(errormsg)
        ...
        people = sim.agents.human

        # find the seed infections (set during sim.init_people()) and blank them out
        seed_inds = znu.true(people.date_exposed == 0)
        ...
        for wave in range(n_waves):
            ...
            wave_probs = np.ones(sim['pop_size_by_type']['human']) * self.prob[wave]
            ...
            this_wave_inds = znu.true(znu.binomial_arr(wave_probs)) # Finally, calculate who actually was infected
            ...
            this_inf_offset_days = znu.sample(**self.dist[wave], size=len(this_wave_inds)) - days_prior

            # require that all offsets are before the start of the sim
            filtered_wave_inds = znu.true(this_inf_offset_days <= 0)
            ...
            inf_offset_days = inf_offset_days + np.round(this_inf_offset_days[filtered_wave_inds]).astype(znd.default_int).tolist()
        ...
        new_nab_length = sim.npts - np.floor(np.min(inf_offset_days)).astype(znd.default_int)
        if new_nab_length > len(sim.pars['immunity_pars']['human']['nab_kin']):
            sim.pars['immunity_pars']['human']['nab_kin'] = znimm.precompute_waning(length=new_nab_length, pars=sim.pars['immunity_pars']['human']['nab_decay'])
            people.pars['immunity_pars']['human']['nab_kin'] = sim.pars['immunity_pars']['human']['nab_kin']
        ...
        for t in np.arange(np.min(inf_offset_days), 0):
            ...
            for wave in range(n_waves):
                inds = znu.true(np.logical_and(inf_offset_days == t, wave_id == wave))
            ...
            # this will mess up those who are the seed infections if not reset to naive (see above)
            sim.agents.human.t = t
            has_nabs = znu.true(sim.agents.human.peak_nab)
            if len(has_nabs):
                znimm.update_nab(sim.agents.human, inds=has_nabs)

        # update states for t=0
        people.update_states_pre(t=0)

        # reset the seed infections
        people.infect(seed_inds, layer='seed_infection')
