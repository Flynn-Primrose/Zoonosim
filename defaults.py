...
import numba as nb
import sciris as sc
from .settings import options as zno # To set options
...
def get_default_plots(which='default', kind='sim', sim=None):
    ...
    if which in ['none', 'default', 'multi-pathogen', 'multi-pathogen-simple']:

        if is_sim and which != ('multi-pathogen') and which != ('multi-pathogen-simple'):
            plots = sc.odict({
                'Total counts': [
                    'cum_infections',
                    'n_infectious',
