'''
Defines the Poultry class and functions associated with making Poultry and handling
...
class Poultry(cvb.BaseRoster):
    '''
    A class to perform all the operations on the Poultry -- usually not invoked directly.

    This class is usually created automatically by the sim. The only required input
    argument is the population size, but typically the full parameters dictionary
    will get passed instead since it will be needed before the People object is
    initialized. However, ages, contacts, etc. will need to be created separately --
    see ``zn.make_poultry()`` instead.
    ...
    Args:
        ...
        strict (bool): whether or not to only create keys that are already in self.meta.poultry; otherwise, let any key be set
        kwargs (dict): the actual data, e.g. from a popdict, being specified

    **Examples**::

        pltr1 = zn.Poultry(2000)

        sim = zn.Sim()
        pltr2 = zn.Poultry(sim.pars)
    '''
    ...
    def __init__(self, pars, strict=True, **kwargs):
        ...
        self.meta = znconfig.PoultryMeta() # Store list of keys and dtypes
        self.contacts = None
    ...
    def check_diagnosed(self):
        ...
        diag_expired_inds = cvu.true(self.t - self.date_diagnosed > 14)
