...
def handle_args(fig_args=None, plot_args=None, scatter_args=None, axis_args=None, fill_args=None,
    ...
    defaults.style   = sc.objdict(style=None, dpi=None, font=None, fontsize=None, grid=None, facecolor=None) # Use Zoonosim global defaults
...
def handle_to_plot(kind, to_plot, n_cols, sim, check_ready=True):
    ...
    if to_plot is None or isinstance(to_plot, str):
        to_plot = znd.get_default_plots(to_plot, kind=kind, sim=sim)
    ...
    n_rows,n_cols = sc.get_rows_cols(n_plots, ncols=n_cols) # Inconsistent naming due to Zoonosim/Matplotlib conventions
...
def reset_ticks(ax, sim=None, date_args=None, start_day=None, n_cols=1):
    ...
    if d_args['as_dates']:
        ...
        if d_args['dateformat'] in ['zoonosim', 'sciris', 'auto', 'matplotlib', 'concise', 'brief']: # Handle date formatter rather than date format
            style, dateformat = d_args['dateformat'], None # Swap argument order
            style = style.replace('zoonosim', 'sciris') # In case any users are confused about what "default" is
        else:
...
def tidy_up(fig, figs, sep_figs, do_save, fig_path, do_show, args):
    ...
    if do_save:
        ...
        znm.savefig(fig=figlist, filename=fig_path) # Save the figure
                ...
                for p in range(len(sim.pathogens)):
                    ...
                    if reskey == 'co-infections':
                        ...
                        if reskey in variant_keys: 
                            ...
                            variant_colors = sc.gridcolors(ns+p)
                            for variant in range(ns):
                                # Colors and labels
                                v_color = variant_colors[variant+p]
                                p_label = sim['pathogens'][p].label
                                v_label = (f'{p_label} wild type') if variant == 0 else sim['pathogens'][p].variants[variant-1].label
                                color = set_line_options(colors,f"{sim.pathogens[p].label}: reskey", resnum, v_color)  # Choose the color
                                label = set_line_options(labels, f"{sim.pathogens[p].label}: reskey", resnum, '')  # Choose the label
...
def plot_result(key, sim=None, fig_args=None, plot_args=None, axis_args=None, scatter_args=None,
    ...
    ''' Plot a single result -- see ``zn.Sim.plot_result()`` for documentation '''
