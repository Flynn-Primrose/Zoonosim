...
def make_community_contacts(pop, structs, pars, popdict):
    ...
    for uid in range(n_uids):
        ...
        if pars.com_dispersion is None:
            n_contacts = znu.n_poisson(pars.com_contacts, 1)[0]
        else:
            n_contacts = znu.n_neg_binomial(pars.com_contacts, pars.com_dispersion, 1)[0]
        
        n_contacts = int(n_contacts/2.0) # Contacts counted twice in Covasim engine.

        # Randomly select N people from the population. (UID's)
        popdict[uid]['contacts']['C'] = set(znu.choose_r(max_n=n_uids,n=n_contacts))
...
def make_school_contacts(pop, structs, pars, popdict):
    ...
    for ns, students in enumerate(structs.student_uid_lists):
        ...
        if pars.with_school_types:
            ...
            popdict, student_groups, teacher_groups = znsch.add_school_edges(popdict, students, student_ages,
            ...
            school_edges = znsch.generate_random_contacts_across_school(school, pars.average_class_size)
            popdict = znsch.add_contacts_from_edgelist(popdict, school_edges, 'S')
