    ...
    **Example**::
    ...
    def parse(self, variant=None, label=None):
        ...
        if isinstance(variant, str):
            ...
            choices, mapping = znpar.get_variant_choices()
            known_variant_pars = znpar.get_variant_pars()
        ...
        # Option 2: variants can be specified as a dict of pars
            ...
            default_variant_pars = znpar.get_variant_pars(default=True)
            ...
            for key in variant_pars.keys():
                if key not in default_keys:
                    invalid.append(key)
            if len(invalid):
                errormsg = f'Could not parse variant keys "{sc.strjoin(invalid)}"; valid keys are: "{sc.strjoin(znd.variant_pars)}"'
    ...
    def initialize(self, sim):
        ...
        self.days = zni.process_days(sim, self.days) # Convert days into correct format
...
def update_peak_nab(people, inds, nab_pars, symp=None):
    ...
    if len(no_prior_nab_inds):
        ...
        init_nab = znu.sample(**nab_pars['nab_init'], size=len(no_prior_nab_inds))
...
def check_immunity(people, variant):
    '''
    Calculate agents's immunity on this timestep from prior infections + vaccination. Calculates effective NAbs by
    ...
    is_vacc = znu.true(people.vaccinated)  # Vaccinated
    vacc_source = people.vaccine_source[is_vacc]
    was_inf = znu.true(people.t >= people.date_recovered)  # Had a previous exposure, now recovered
    was_inf_same = znu.true((people.recovered_variant == variant) & (people.t >= date_rec))  # Had a previous exposure to the same variant, now recovered
    ...
    variant_was_inf_diff = variant_was_inf_diff.astype(znd.default_int)
...
def nab_growth_decay(length, growth_time, decay_rate1, decay_time1, decay_rate2, decay_time2):
    ...
    def f2(t, decay_time1, decay_time2, decay_rate1, decay_rate2):
        ...
        decayRate[znu.true(t>decay_time2)] = decay_rate2
        slowing = (1 / (decay_time2 - decay_time1)) * (decay_rate1 - decay_rate2)
        decayRate[znu.true((t>decay_time1)*(t<=decay_time2))] = decay_rate1 - slowing * (np.arange(len(znu.true((t>decay_time1)*(t<=decay_time2))), dtype=znd.default_int))
    ...
    length = length + 1
    t1 = np.arange(growth_time, dtype=znd.default_int)
    t2 = np.arange(length - growth_time, dtype=znd.default_int)
...
def nab_decay(length, decay_rate1, decay_time1, decay_rate2):
    ...
    t  = np.arange(length, dtype=znd.default_int)
    y1 = f1(znu.true(t<=decay_time1), decay_rate1)
    y2 = f2(znu.true(t>decay_time1), decay_rate1, decay_time1, decay_rate2)
...
def exp_decay(length, init_val, half_life, delay=None):
    ...
    if delay is not None:
        t = np.arange(length-delay, dtype=znd.default_int)
