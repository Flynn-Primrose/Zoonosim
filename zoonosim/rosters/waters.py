'''
Defines the subroster and metaroster for water objects.
'''
import numpy as np
import sciris as sc
...
class Water(Subroster):
    ...
    def set_rel_trans(self):
        ...
        if self.pars['transmission_pars']['water']['beta_dist'] is not None:
            self.rel_trans = self.rel_trans * znu.sample(**self.pars['transmission_pars']['water']['beta_dist'], size = len(self))
        return
