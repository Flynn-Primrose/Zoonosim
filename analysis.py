...
class age_histogram(Analyzer):
    ...
    **Examples**::
    ...
    def initialize(self, sim):
        ...
        if self.datafile is not None:
            if sc.isstring(self.datafile):
                self.data = znm.load_data(self.datafile, check_date=False)
            else:
    ...
    def plot(self, windows=False, width=0.8, color='#F8A493', fig_args=None, axis_args=None, data_args=None, **kwargs):
        ...
        return znpl.handle_show_return(figs=figs)
...
class daily_age_stats(Analyzer):
    ...
    **Examples**::
    ...
    def plot(self, total=False, do_show=None, fig_args=None, axis_args=None, plot_args=None,
        ...
        return znpl.handle_show_return(fig=fig, do_show=do_show)
    ...
    **Example**::
    ...
    def apply(self, sim):
        for ind in zni.find_day(self.days, sim.t):
            ...
            # Transmission stats
            newinfs = znu.true(ppl.date_exposed == sim.t)
            ...
            # Testing stats
            newtests = znu.true(ppl.date_tested == sim.t)
            ...
            # Quarantine stats
            q_inds = np.union1d(self.inds['quarantined'], znu.true(ppl.date_end_quarantine == sim.t)) # Append people who finished quarantine today
            eq_inds = znu.true(ppl.date_quarantined == sim.t-1) # People entering quarantine the day before (their first full day of quarantine)
            fq_inds = znu.true(ppl.date_end_quarantine == sim.t+1) # People finishing quarantine; +1 since on the date of quarantine end, they are released back and can get infected at normal rates
...
class nab_histogram(Analyzer):
    ...
    **Example**::
    ...
    def initialize(self, sim):
        ...
        if self.days is None:
            self.days = self.end_day  # If no day is supplied, use the last day
        self.days, self.dates = zni.process_days(sim, self.days,
                ...
                if key in self.keys:
                    for i,ax in enumerate([main_ax1, main_ax2]):
                        ...
                        if k == len(self.keys)-1:
                            ...
                            ax.set_title(title)
                            znpl.reset_ticks(ax=ax, date_args=date_args, start_day=self.sim_results['date'][0])
                            ax.legend()
