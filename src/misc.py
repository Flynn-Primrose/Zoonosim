...
def load_data(datafile, calculate=True, check_date=True, verbose=True, start_day=None, **kwargs):
    ...
    if isinstance(datafile, str):
        ...
        if df_lower.endswith('csv'):
            ...
            data = pd.read_excel(datafile, **kwargs, engine='openpyxl')
...
def savefig(filename=None, comments=None, fig=None, **kwargs):
    ...
    Zoonosim metadata in the figure.

    By default, saves (git) information from both the Zoonosim version and the calling
    function. Additional comments can be added to the saved file as well. These can
    be retrieved via ``zn.get_png_metadata()`` (or ``sc.loadmetadata``). Metadata can
    ...
    **Example**::
    ...
    metadata = {}
    metadata['Zoonosim version'] = znv.__version__
    gitinfo = git_info()
    for key,value in gitinfo['zoonosim'].items():
        metadata[f'Zoonosim {key}'] = value
    for key,value in gitinfo['called_by'].items():
        metadata[f'Zoonosim caller {key}'] = value
    metadata['Zoonosim current time'] = sc.getdate()
    metadata['Zoonosim calling file'] = sc.getcaller()
    if comments:
        metadata['Zoonosim comments'] = comments
...
def git_info(filename=None, check=False, comments=None, old_info=None, die=False, indent=2, verbose=True, frame=2, **kwargs):
    ...
    **Examples**::
        ...
        zn.git_info() # Return information
        zn.git_info(__file__) # Writes to disk
        zn.git_info('zoonosim_version.gitinfo') # Writes to disk
        zn.git_info('zoonosim_version.gitinfo', check=True) # Checks that current version matches saved file
    '''
    ...
    cv_info = {'version':znv.__version__}
    ...
    info = {'zoonosim':cv_info, 'called_by':caller_info}
    if comments:
    ...
    # Check if versions match, and optionally raise an error
        ...
        string = ''
        old_cv_info = old_info['zoonosim'] if 'zoonosim' in old_info else old_info
...
def check_version(expected, die=False, verbose=True):
    ...
    **Example**::
    ...
    if relation: # Versions mismatch, print warning or raise error
        string = f'Note: Zoonosim is {relation} than expected ({version} vs. {expected})'
...
def check_save_version(expected=None, filename=None, die=False, verbose=True, **kwargs):
    ...
    which version of Zoonosim was used.
...
def get_version_pars(version, verbose=True):
    ...
    Parameters will be loaded for Zoonosim 'as at' the requested version i.e. the
    ...
    - If parameters for version '1.3.5' are requested, parameters will be returned from '1.3', since
      Zoonosim at version 1.3.5 would have been using the parameters defined at version 1.3.
    ...
    available_versions = sorted(available_versions, key=lambda x: sc.compareversions(x, version)) # AI suggested completion.
...
def get_png_metadata(filename, output=False):
    ...
    **Example**::
        ...
        zn.Sim().run(do_plot=True)
        zn.savefig('Zoonosim.png')
        zn.get_png_metadata('Zoonosim.png')
    '''
    try:
        import PIL
...
def help(pattern=None, source=False, ignorecase=True, flags=None, context=False, output=False):
    ...
    **Examples**::
        ...
        zn.help()
        zn.help('vaccine')
        zn.help('contact', ignorecase=False, context=True)
        zn.help('lognormal', source=True, context=True)

    | New in version 3.1.2.
    '''
    defaultmsg = '''
To search for a keyword/phrase/regex in Zoonosim's docstrings, use e.g.:

    >>> zn.help('vaccine')

See help(zn.help) for more information.

For help on Zoonosim options, see zn.options.help().
'''
    ...
    if pattern is None:
        ...
        funcs = [funcname for funcname in dir(zn) if func_ok(funcname, getattr(zn, funcname))] # Skip dunder methods and modules

        # Get docstrings or full source code
        docstrings = dict()
        for funcname in funcs:
            f = getattr(zn, funcname)
